...
st.markdown('**Random Forest Classification - Real Data**')
...
def load_models():
    try:
        ...
        scaler = joblib.load("models/scaler_rand_real.joblib")
...
if st.button("Submit"):
    st.subheader("Random Forest Classifier Prediction - Real Data")

    input_data = pd.DataFrame([{
        'num_pages': num_pages,
        'ratings_count': ratings_count,
        'book_age_days': book_age_days,
        'language_code_encoded': int(df_real['language_code_encoded'].median()),
        'author_encoded': int(df_real['author_encoded'].median())
    }])
    ...
    filter_books = df_real[
        (df_real['num_pages'].between(num_pages * 0.7, num_pages * 1.3)) &
        (df_real['ratings_count'].between(ratings_count * 0.5, ratings_count * 2)) &
        (df_real['book_age_days'].between(book_age_days * 0.7, book_age_days * 1.3)) &
        (df_real['rating_category'] == predict_cat)
    ].copy()
